...
function [f,gradf]=rrOracle(b,w,s,data,atZero)

% Compute the (unbalanced) squared loss function and its gradient
...
% Loss
f = minuserr'*minuserr/n;

% Block gradient
if s
    gradf = 2*x(:,groups{s})'*minuserr/n;
else
    gradf = 2*sum(minuserr)/n;
end